...
public class UserGameRoute : RouteService
{
    public override async Task Route(PayloadStructure payloadStructure)
    {
        if (payloadStructure.TableCode != (int)TableEnum.UserGames)
        {
        ...
        var adminData = await _databaseService.GetFromRowId<UserGame>(payloadStructure.RowId, cts.Token);
