...
public class BaseModelService : IDisposable
{
    ...
    [Inject] protected DatabaseService? DatabaseService;
    ...
    /// <summary>
    /// Vurtial class for childrens with initialize logic.
    /// In default calling InitializeDiContainer and check nullability
    /// </summary>
    ...
    protected bool IsDataNotNull(object? data)
    {
        if (data is null)
        {
            Logger.Error($"Data with type {data?.GetType()} is null");
            return false;
        }
